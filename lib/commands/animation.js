...
/**
 * Change the state of animation on the device under test via adb settings command or settings app.
...
export async function setAnimationState (on) {
  ...
  await this.adb.getApiLevel() >= 26
    ? await this.adb.setAnimationScale(on ? 1 : 0)
    // Please delete AnimationSettingHandler.java implementation also to avoid non-sdk https://developer.android.com/guide/app-compatibility/restrictions-non-sdk-interfaces
    // error in settings app when we drop api level 25 and lower.
